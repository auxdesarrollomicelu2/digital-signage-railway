...
export default function DeleteModal({ open, onClose, onConfirm, itemName }) {
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      <div className="sweep" style={{ '--sc': T.red, '--so': 1, position: 'relative', zIndex: 1, width: '100%', maxWidth: 400, maxHeight: '90vh', overflowY: 'auto', borderRadius: 20, animation: 'modalIn .3s cubic-bezier(.22,1,.36,1) both' }}>
        <div style={{ background: T.modalBg, borderRadius: 19, border: `1px solid ${T.border}`, overflow: 'hidden', textAlign: 'center', padding: '36px 28px 28px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Btn variant="primary" accentColor={T.red} onClick={onConfirm} style={{ flex: 1, padding: '11px' }}><Trash2 size={13} /> Eliminar</Btn>
          </div>
        </div>
      </div>
    </div>,
